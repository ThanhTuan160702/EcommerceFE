...
const PaginationUser = ({totalPage, currentPage, handleChangeCurrentPage, handleCurrentPage}) => {
    ...
    return (
        <div className='py-2 flex gap-2 items-center'>
          ...
          <button onClick={() => handleChangeCurrentPage('+')} className='py-1 cursor-pointer px-2 borde' disabled={currentPage===Math.ceil(totalPage/10)}>
              <IoIosArrowForward />
          </button>
          <span>{Math.ceil(totalPage/10) < 1 ? 1 : Math.ceil(totalPage/10)}</span>
        </div>
      )
}
